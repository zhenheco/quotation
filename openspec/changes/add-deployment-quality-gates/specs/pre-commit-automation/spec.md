...
**Then** pre-commit hook 應該自動執行 ESLint 檢查
**And** 只檢查 staged files（提升效能）
**And** 如果發現錯誤應該阻止 commit

#### Scenario: Commit 前自動執行 typecheck

**Given** 開發者修改了 TypeScript 檔案
**When** 執行 `git commit`
**Then** pre-commit hook 應該執行型別檢查
**And** 如果發現型別錯誤應該阻止 commit
**And** 錯誤訊息應該指出具體的錯誤位置
...
**When** 執行 `git commit`
**Then** pre-commit hook 應該自動執行 `eslint --fix`
**And** 修復後的檔案應該自動加入 staging area
**And** commit 應該繼續進行
...
**Then** Husky 應該自動初始化 git hooks
**And** .husky/pre-commit 檔案應該存在
**And** 該檔案應該是可執行的
...
**Then** 新的 hook 配置應該立即生效
**And** 下次 commit 時應該使用新的檢查邏輯
...
**When** 執行 `git commit`
**Then** lint-staged 應該對不同檔案類型執行對應的檢查
...
**Then** pre-commit hook 應該檢測到 lockfile 不同步
**And** 應該自動執行 `pnpm install --frozen-lockfile --lockfile-only`
**And** 更新後的 lockfile 應該自動加入 staging area
...
**When** 執行 `git commit`
**Then** pre-commit hook 應該通過檢查
**And** 兩個檔案都應該被成功 commit
...
**When** 執行 `git commit`
**Then** TypeScript 和 ESLint 檢查應該被跳過
**And** commit 應該快速完成
