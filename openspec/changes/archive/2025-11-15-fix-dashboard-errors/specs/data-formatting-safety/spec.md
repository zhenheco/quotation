...
1. **app/[locale]/dashboard/DashboardClient.tsx**
   - Line 148: `item.amount.toLocaleString()`
   - Line 171-173: `formatCurrency` 函式

2. **components/DashboardCharts.tsx**
   - Line 48: `amount.toLocaleString()` (可能需要檢查)
   - Line 64: `formatCurrency` 使用

3. **lib/utils/format.ts**
   - `formatPrice` 和 `formatCurrency` 函式

### 實作策略

#### 選項 1: 使用 Nullish Coalescing
```typescript
const displayAmount = (amount ?? 0).toLocaleString()
```

#### 選項 2: 使用可選鏈和預設值
```typescript
const displayAmount = amount?.toLocaleString() ?? 'N/A'
```

#### 選項 3: 建立統一的格式化函式
```typescript
function safeFormatNumber(value: number | null | undefined, defaultValue = 0): string {
...
describe('formatCurrency', () => {
  it('should format valid numbers', () => {
    expect(formatCurrency(1234.56)).toBe('TWD 1,234.56')
  })
...
## Acceptance Criteria
- [ ] 所有 `toLocaleString()` 呼叫前都有 null/undefined 檢查
- [ ] `formatCurrency` 函式可安全處理無效值
- [ ] Console 無 `TypeError` 相關錯誤
- [ ] 所有統計數據正確顯示
- [ ] 通過單元測試
- [ ] 通過 TypeScript 類型檢查
