-- ============================================================================
-- Migration: 049_rewrite_accounting_rpc.sql
-- Created: 2025-12-26
-- Description: 重寫會計系統 RPC 函數，與實際表結構 (044 + 048) 一致
-- ============================================================================
...
DECLARE
  ...
  v_tx JSONB;
  v_source_type transaction_source;
  v_result JSONB;
BEGIN
  -- 解析日期
  v_date := COALESCE((p_journal_data->>'date')::DATE, CURRENT_DATE);
  v_year := EXTRACT(YEAR FROM v_date);
  v_month := EXTRACT(MONTH FROM v_date);

  -- 產生傳票編號
  SELECT COALESCE(MAX(SUBSTRING(journal_number FROM 7)::INTEGER), 0) + 1
  INTO v_seq
  ...
  v_journal_number := FORMAT('%04d%02d%04d', v_year, v_month, v_seq);
  ...
  v_source_type := COALESCE(
    (p_journal_data->>'source_type')::transaction_source,
  ...
  INSERT INTO journal_entries (
    ...
    v_source_type,
    (p_journal_data->>'invoice_id')::UUID,
    'DRAFT'::journal_status,
    COALESCE((p_journal_data->>'is_auto_generated')::BOOLEAN, false)
  )
...
COMMENT ON FUNCTION create_journal_with_transactions IS '建立會計傳票與分錄（原子性操作）- 修正版';
...
COMMENT ON FUNCTION post_invoice_with_journal IS '過帳發票並自動產生會計傳票 - 修正版';
...
COMMENT ON FUNCTION void_invoice IS '作廢發票及相關傳票 - 修正版';
...
COMMENT ON FUNCTION post_journal_entry IS '過帳會計傳票 - 修正版';
...
COMMENT ON FUNCTION void_journal_entry IS '作廢會計傳票 - 修正版';
...
BEGIN
  ...
  SELECT
    ...
    a.category,
    COALESCE(SUM(t.debit), 0)::NUMERIC(15,2) AS debit_total,
    COALESCE(SUM(t.credit), 0)::NUMERIC(15,2) AS credit_total,
    (COALESCE(SUM(t.debit), 0) - COALESCE(SUM(t.credit), 0))::NUMERIC(15,2) AS balance
  FROM accounts a
  LEFT JOIN acc_transactions t ON t.account_id = a.id
    AND t.status = 'POSTED'
...
COMMENT ON FUNCTION get_trial_balance IS '取得試算表 - 修正版';
...
COMMENT ON FUNCTION match_bank_transaction IS '銀行交易與傳票配對 - 修正版';
...
BEGIN
  ...
  IF v_new_paid > v_invoice.total_amount THEN
    RAISE EXCEPTION '付款金額超過發票總額：總額 %，已付 %，本次 %',
      v_invoice.total_amount, v_invoice.paid_amount, p_amount;
  END IF;
  ...
  -- 建立付款記錄（使用新建的 invoice_payments 表）
  ...
  SET
    ...
    payment_method = COALESCE(p_payment_method::payment_method_type, payment_method),
    updated_at = NOW()
  WHERE id = p_invoice_id;
...
COMMENT ON FUNCTION record_invoice_payment IS '記錄發票付款 - 修正版';
...
VALUES ('049_rewrite_accounting_rpc.sql')
ON CONFLICT (filename) DO NOTHING;

NOTIFY pgrst, 'reload schema';

SELECT 'Migration 049: All accounting RPC functions rewritten!' as status;
