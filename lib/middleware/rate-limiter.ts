...
/**
 * 使用 Cloudflare KV 檢查速率限制（分散式環境）
 *
 * 相比記憶體 LRU 版本的優點：
 * 1. 跨所有 Workers 實例共享狀態
 * 2. 資料持久化，不會因 Worker 重啟而丟失
 * 3. 全球邊緣節點，低延遲
 *
...
export async function checkRateLimitKV(
  ...
  try {
    // 從 KV 取得目前計數
    const data = await kv.get(key, 'json') as KVRateLimitData | null
    ...
    if (!data || now > data.resetAt) {
      ...
      await kv.put(key, JSON.stringify(newData), {
        expirationTtl: Math.ceil(windowMs / 1000) + 60 // 額外 60 秒緩衝
      })

      return {
        limited: false,
        remaining: maxRequests - 1,
        resetTime: new Date(newData.resetAt).toISOString()
      }
    }
    ...
    if (data.count >= maxRequests) {
      ...
      logger.warn('Rate limit exceeded (KV)', {
        ...
        count: data.count,
        limit: maxRequests,
        resetTime: new Date(data.resetAt).toISOString()
      })
    ...
    // 增加計數
    const updatedData: KVRateLimitData = {
      count: data.count + 1,
    ...
    return {
      limited: false,
      remaining: maxRequests - updatedData.count,
      resetTime: new Date(data.resetAt).toISOString()
    }
  } catch (error) {
    // KV 錯誤時降級到允許請求（fail open）
    logger.error('KV rate limit error, falling back to allow', {
