...
export interface AccInvoice {
  ...
  // 申報期別（Migration 086 新增）
  declared_period_id: string | null;
...
async function getDeclarationStatus(
  ...
  if (error && error.code !== "PGRST116") {
    throw new Error(`查詢申報期別狀態失敗: ${error.message}`);
  }
...
export async function getInvoiceSummary(
  ...
  return invoices.reduce(
    (acc, inv) => ({
      ...
      total_untaxed:
        acc.total_untaxed + (parseFloat(String(inv.untaxed_amount)) || 0),
      total_tax: acc.total_tax + (parseFloat(String(inv.tax_amount)) || 0),
      total_amount:
        acc.total_amount + (parseFloat(String(inv.total_amount)) || 0),
...
export async function updateInvoice(
  ...
  if (!isDraft) {
    ...
    if (attemptedRestrictedFields.length > 0) {
      throw new Error(
        `非草稿狀態只能修改備註和到期日，不能修改: ${attemptedRestrictedFields.join(", ")}`,
      );
    }
  } else {
    // 草稿狀態可編輯所有欄位
    ...
    if (input.untaxed_amount !== undefined)
      updateData.untaxed_amount = input.untaxed_amount;
    if (input.tax_amount !== undefined)
      updateData.tax_amount = input.tax_amount;
    if (input.total_amount !== undefined)
      updateData.total_amount = input.total_amount;
    if (input.counterparty_id !== undefined)
      updateData.counterparty_id = input.counterparty_id;
    if (input.counterparty_tax_id !== undefined)
      updateData.counterparty_tax_id = input.counterparty_tax_id;
    if (input.counterparty_name !== undefined)
      updateData.counterparty_name = input.counterparty_name;
    if (input.tax_code_id !== undefined)
      updateData.tax_code_id = input.tax_code_id;
    if (input.description !== undefined)
      updateData.description = input.description;
    if (input.account_id !== undefined)
      updateData.account_id = input.account_id;
    if (input.account_code !== undefined)
      updateData.account_code = input.account_code;
    if (input.due_date !== undefined) updateData.due_date = input.due_date;
    if (input.attachment_url !== undefined)
      updateData.attachment_url = input.attachment_url;
...
export async function deleteInvoice(
  ...
  if (existing.declared_period_id) {
    ...
    if (
      ...
      throw new Error("此發票所屬申報期別已送出或結案，不能刪除");
    }
  }
