...
export default function InvoiceDetailClient({ invoiceId }: InvoiceDetailClientProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          {invoice.status === 'DRAFT' && (
            ...
            >
              {verifyInvoice.isPending ? '處理中...' : '審核'}
            </Button>
          )}
          {/* 過帳按鈕（已審核狀態） */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              {invoice.counterparty_tax_id && (
                <p className="text-sm text-muted-foreground">
                  統一編號: {invoice.counterparty_tax_id}
                </p>
              )}
            </div>
            ...
            <div>
              ...
              {invoice.account_confidence && (
                <p className="text-sm text-muted-foreground">
                  信心度: {Math.round(invoice.account_confidence * 100)}%
                </p>
              )}
            </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>金額明細</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            <div className="flex justify-between py-2 border-b">
              <span className="text-muted-foreground">未稅金額</span>
            ...
            <div className="flex justify-between py-2 text-lg">
              <span className="font-medium">含稅金額</span>
      ...
      {invoice.journal_entry_id && (
        <Card>
          <CardHeader>
            <CardTitle>相關傳票</CardTitle>
          </CardHeader>
          <CardContent>
            <Button variant="outline" asChild>
              <Link href={`/accounting/journals/${invoice.journal_entry_id}`}>
                查看傳票
              </Link>
            </Button>
          </CardContent>
