...
export default function BusinessCardScanner({ onScanComplete, disabled }: BusinessCardScannerProps) {
  ...
  const handleFileSelect = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!result.success) {
        throw new Error(result.error || '名片掃描失敗')
      }

      if (result.data) {
        onScanComplete(result.data)
        toast.success('名片識別成功')
      } else {
        toast.warning('未識別到任何資料')
      }

    } catch (error) {
      console.error('名片掃描失敗:', error)
      toast.error(error instanceof Error ? error.message : '名片掃描失敗')
    } finally {
      setIsScanning(false)
      // 清除 input 以便重複選擇同一檔案
  ...
  return (
    <div className="mb-4">
      ...
      <p className="mt-1 text-xs text-gray-500">拍攝或上傳名片，自動識別聯絡資訊</p>
    </div>
  )
}
