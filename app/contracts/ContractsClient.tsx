...
export default function ContractsClient({ locale }: ContractsClientProps) {
        ...
        {overdueCount > 0 && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            <div className="flex items-center gap-2 text-red-800">
              ...
              <div>
                <h3 className="font-semibold">逾期提醒</h3>
                <p className="text-sm">{overdueCount} 筆合約已逾期收款</p>
              </div>
            </div>
          </div>
        )}
        ...
        {expiringCount > 0 && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <div className="flex items-center gap-2 text-yellow-800">
              ...
              <div>
                <h3 className="font-semibold">即將到期</h3>
                <p className="text-sm">{expiringCount} 筆合約即將到期</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex gap-2">
            {(['all', 'active', 'expired', 'overdue'] as const).map((status) => (
              ...
              >
                {status === 'all' ? '全部' : status === 'active' ? '執行中' : status === 'expired' ? '已到期' : '逾期'}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredContracts.map((contract) => (
            <ContractCard
              ...
              onRecordPayment={() => {
                window.location.href = `/payments?contract_id=${contract.id}`
              }}
              onSendReminder={() => {
                toast.info('提醒已發送')
              }}
