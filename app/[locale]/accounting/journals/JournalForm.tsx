 ...
 */
export default function JournalForm({ locale }: JournalFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/${locale}/accounting/journals`)
    } catch (error) {
    ...
    <form onSubmit={handleSubmit}>
        ...
        <CardContent>
          ...
          {!isBalanced && (
            <div className="mt-2 text-sm text-destructive text-right">
              {t('accounting.form.imbalanced')}: {Math.abs(totals.debit - totals.credit).toLocaleString('zh-TW')}
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <div className="flex justify-end gap-3">
        <Button
          type="button"
          variant="outline"
          onClick={() => router.push(`/${locale}/accounting/journals`)}
