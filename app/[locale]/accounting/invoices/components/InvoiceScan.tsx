 ...
 */
export default function InvoiceScan({ locale, onSuccess }: InvoiceScanProps) {
  ...
  const handleFileSelect = useCallback(
    async (file: File) => {
      ...
      if (!validTypes.includes(file.type)) {
        toast.error(t('accounting.scan.invalidFileType'))
        return
      }
      ...
      if (file.size > maxSize) {
        toast.error(t('accounting.scan.fileTooLarge'))
        return
      }
      ...
      try {
        ...
        setStep('review')
        toast.success(t('accounting.scan.scanSuccess'))
      } catch (error) {
        console.error('Scan error:', error)
        const message = error instanceof Error ? error.message : t('accounting.scan.scanError')
        toast.error(message)
        setStep('upload')
      }
    },
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          <div>
            <p className="text-slate-700 font-medium">{t('accounting.scan.dragDropHint')}</p>
            <p className="text-slate-500 text-sm mt-1">{t('accounting.scan.orClickToSelect')}</p>
          </div>
          <p className="text-slate-400 text-xs">{t('accounting.scan.acceptedFormats')}</p>
        </div>
      </div>

      {/* 提示說明 */}
      <div className="bg-blue-50 rounded-xl p-4">
        <h4 className="font-medium text-blue-700 mb-2">{t('accounting.scan.tips')}</h4>
        <ul className="text-sm text-blue-600 space-y-1">
          <li>{t('accounting.scan.tip1')}</li>
          <li>{t('accounting.scan.tip2')}</li>
          <li>{t('accounting.scan.tip3')}</li>
        </ul>
      </div>
    </div>
  )
  ...
  const renderScanning = () => (
    <div className="flex flex-col items-center justify-center py-12">
      ...
      <p className="text-lg font-medium text-slate-700">{t('accounting.scan.scanning')}</p>
      <p className="text-slate-500 mt-2">{t('accounting.scan.pleaseWait')}</p>
    </div>
  )
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-4 bg-slate-50 rounded-xl p-4">
        ...
        <div className="flex-1">
          <h4 className="font-medium text-slate-700">{t('accounting.scan.reviewTitle')}</h4>
          <p className="text-sm text-slate-500 mt-1">{t('accounting.scan.reviewDescription')}</p>
          {fieldsNeedingReview.length > 0 && (
            <div className="mt-2 flex items-center gap-2">
              <span className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-amber-100 text-amber-700">
                ...
                {t('accounting.scan.lowConfidenceFields', { count: fieldsNeedingReview.length })}
              </span>
            </div>
          )}
        </div>
        <Button variant="outline" size="sm" onClick={handleReset}>
          {t('accounting.scan.rescan')}
        </Button>
      </div>
  ...
  const renderSaving = () => (
    <div className="flex flex-col items-center justify-center py-12">
      ...
      <p className="mt-4 text-lg font-medium text-slate-700">{t('common.saving')}</p>
    </div>
  )
