...
export default function SupplierList({ locale }: SupplierListProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : t('supplier.deleteError'))
    }
  }
    ...
    return (
      <div className="p-6">
        <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg">
          {error instanceof Error ? error.message : t('supplier.fetchError')}
        </div>
      </div>
    )
  }
  ...
  if (!suppliers || suppliers.length === 0) {
    return (
      <EmptyState
        icon="🏭"
        title={t('supplier.emptyState.title')}
        description={t('supplier.emptyState.description')}
        action={{
          label: t('supplier.createNew'),
          onClick: () => router.push(`/${locale}/suppliers/new`),
        }}
      />
    )
  }
  ...
  return (
    <>
                    ...
                    <tr key={supplier.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="text-sm text-gray-900">
                          {supplier.payment_terms || (supplier.payment_days ? `${supplier.payment_days} ${t('supplier.days')}` : '-')}
                        </div>
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                        <button
                          onClick={() => router.push(`/${locale}/suppliers/${supplier.id}`)}
              ...
              return (
                ...
                >
                  <div className="mb-3">
                    ...
                    {supplier.code && (
                      <p className="text-xs text-gray-500 mb-1">{t('supplier.code')}: {supplier.code}</p>
                    )}
                    {supplier.supplier_number && (
                    ...
                    <div className="space-y-1.5 text-sm text-gray-600">
                      ...
                      {address?.[locale as 'zh' | 'en'] && (
                        <div className="flex items-start gap-2">
                          ...
                          <span className="line-clamp-2">{address[locale as 'zh' | 'en']}</span>
                        </div>
                      )}
                      {(supplier.payment_terms || supplier.payment_days) && (
                        <div className="flex items-start gap-2">
                          ...
                          <span>{supplier.payment_terms || `${supplier.payment_days} ${t('supplier.days')}`}</span>
                        </div>
                      )}
                    </div>
                  </div>
      ...
      <DeleteConfirmModal
        ...
        title={t('supplier.deleteConfirm.title')}
        description={t('supplier.deleteConfirm.description')}
