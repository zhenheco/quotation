...
export async function GET() {
  try {
    ...
    // 1. 查詢已過期的付費訂閱（ACTIVE/TRIAL/PAST_DUE 且 current_period_end < now）
    ...
    if (expiredError) {
      throw new Error(
        `Failed to query expired subscriptions: ${expiredError.message}`,
      );
    }
    ...
    for (const sub of toDowngrade) {
      try {
        ...
        if (result.success) {
          console.log(
            `[CRON] Downgraded subscription for company ${sub.company_id}`,
          );
        } else {
          console.error(
            `[CRON] Failed to downgrade company ${sub.company_id}:`,
            result.error,
          );
        }
      } catch (error) {
        console.error(
          `[CRON] Error downgrading company ${sub.company_id}:`,
          error,
        );
        results.push({
          company_id: sub.company_id,
    ...
    for (const sub of toCancel) {
      try {
        ...
        if (result.success) {
          console.log(
            `[CRON] Cancelled subscription for company ${sub.company_id}`,
          );
        } else {
          console.error(
            `[CRON] Failed to cancel company ${sub.company_id}:`,
            result.error,
          );
        }
      } catch (error) {
        console.error(
          `[CRON] Error cancelling company ${sub.company_id}:`,
          error,
        );
        results.push({
          company_id: sub.company_id,
    ...
    console.log(
      `[CRON] Subscription downgrade job completed in ${duration}ms:`,
      {
        total: results.length,
        downgraded: results.filter((r) => r.action === "downgrade" && r.success)
          .length,
        cancelled: results.filter((r) => r.action === "cancel" && r.success)
          .length,
        failed: failCount,
      },
    );
