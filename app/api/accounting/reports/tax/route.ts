...
export const GET = withAuth("reports:read")(async (request, { db }) => {
  ...
  if (!yearStr || !biMonthStr) {
    return NextResponse.json(
      { error: "year and bi_month are required" },
      { status: 400 },
    );
  }
  ...
  if (isNaN(biMonth) || biMonth < 1 || biMonth > 6) {
    return NextResponse.json(
      { error: "bi_month must be between 1 and 6" },
      { status: 400 },
    );
  }
...
export const POST = withAuth("reports:read")(async (request, { db }) => {
  ...
  if (!type || (type !== "OUTPUT" && type !== "INPUT")) {
    return NextResponse.json(
      { error: "type must be OUTPUT or INPUT" },
      { status: 400 },
    );
  }
  ...
  const result = await getInvoiceDetailList(
    db,
    companyId,
    type,
    startDate,
