import { NextResponse } from 'next/server'
import { withAuth } from '@/lib/api/middleware'
import { getCompanyIdFromRequest } from '@/lib/utils/company-context'
import { deliverShipment, getShipmentById } from '@/lib/dal/shipments'

/**
 * 送達請求類型
 */
interface DeliverRequestBody {
  actual_delivery?: string
}

/**
 * POST /api/shipments/[id]/deliver - 標記送達
 *
 * 將出貨單狀態變更為 delivered
 * 只有運送中或待處理狀態的出貨單才能被標記為已送達
 */
export const POST = withAuth('shipments:write')<{ id: string }>(async (request, { db }, { id }) => {
  // 取得公司 ID
  const companyId = getCompanyIdFromRequest(request)
  if (!companyId) {
    return NextResponse.json({ error: 'Company ID required' }, { status: 400 })
  }

  // 檢查出貨單是否存在
  const existingShipment = await getShipmentById(db, companyId, id)
  if (!existingShipment) {
    return NextResponse.json({ error: 'Shipment not found' }, { status: 404 })
  }

  // 取得請求資料
  const body = await request.json() as DeliverRequestBody

  // 標記送達
  const shipment = await deliverShipment(db, companyId, id, body.actual_delivery)

  return NextResponse.json(shipment)
})
