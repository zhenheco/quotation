...
} from "@/lib/services/affiliate-tracking";
...
export async function POST(request: Request) {
  try {
    ...
    // 從 cookie 讀取推薦碼（前端未傳入時使用 cookie）
    if (!body.referral_code) {
      ...
      const cookieHeader = cookieStore
        ...
        .join("; ");
      const cookieRef = getReferralCodeFromCookie(cookieHeader);
      if (cookieRef) {
        body.referral_code = cookieRef;
      }
    }
    ...
    // 計算價格和折扣
    const basePrice =
      body.billing_cycle === "YEARLY" ? plan.yearly_price : plan.monthly_price;
    let discount = 0;
    ...
    const appUrl =
      process.env.NEXT_PUBLIC_APP_URL?.trim() || "https://quote24.cc";
